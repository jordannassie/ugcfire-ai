      ...
      <main style={{ paddingTop: 60, minHeight: '100vh' }}>
        ...
        <div style={{ background: '#060606', borderBottom: `1px solid ${BORDER}`, padding: '48px 24px 44px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            <h1 style={{ fontSize: 'clamp(26px, 3.5vw, 44px)', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.03em', marginBottom: 12, lineHeight: 1.1 }}>
              Open Opportunities
            </h1>
            <p style={{ fontSize: 16, color: 'var(--text-muted)', maxWidth: 520, lineHeight: 1.75 }}>
        ...
        <div style={{ padding: '48px 24px 72px', maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
            <div style={{ fontSize: 13, color: 'var(--text-faint)' }}>{openProjects.length} open projects</div>
            ...
            {openProjects.map(opp => (
              <Link key={opp.id} href={`/opportunities/${opp.id}`} className="opp-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16, flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Deadline: {opp.deadline}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{opp.deliverables.length} deliverable{opp.deliverables.length !== 1 ? 's' : ''} · {opp.contentType}</div>
                </div>
          ...
          <div style={{ textAlign: 'center', background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 20, padding: '40px 24px' }}>
            <h3 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 10 }}>Build your portfolio to unlock projects</h3>
            <p style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 24, maxWidth: 480, margin: '0 auto 24px', lineHeight: 1.7 }}>
