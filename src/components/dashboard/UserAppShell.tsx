...
import { exitDemoMode, getDemoRole } from '@/lib/demoData';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', background: BG, overflow: 'hidden' }}>
      ...
      <nav style={{ height: mobile ? 52 : 52, background: BG, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', padding: '0 14px', gap: mobile ? 0 : 2, flexShrink: 0, zIndex: 100 }}>
        ...
        {!mobile && NAV_LINKS.map(l => {
          ...
          return (
            <Link key={l.label} href={l.href}
              style={{ color: active ? 'var(--text)' : 'var(--text-muted)', fontWeight: active ? 700 : 500, fontSize: 13.5, textDecoration: 'none', padding: '5px 10px', borderBottom: active ? `2px solid ${ORANGE}` : '2px solid transparent', transition: 'color 0.12s', whiteSpace: 'nowrap', fontFamily: 'inherit' }}>
              {l.label}
            </Link>
          );
        })}
        ...
        <div style={{ marginLeft: mobile ? 0 : 'auto', display: 'flex', alignItems: 'center', gap: mobile ? 8 : 8 }}>
          ...
          {/* Search (desktop only) */}
          {!mobile && (
            <div style={{ background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 8, display: 'flex', alignItems: 'center', padding: '5px 10px', gap: 7 }}>
              <Search size={13} color="var(--text-faint)" strokeWidth={2} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ background: 'none', border: 'none', outline: 'none', fontSize: 13, color: 'var(--text-muted)', width: 130, fontFamily: 'inherit' }} />
              <span style={{ fontSize: 11, color: 'var(--text-faint)', background: 'var(--border)', padding: '1px 5px', borderRadius: 3, fontWeight: 500 }}>⌘K</span>
            </div>
          )}
          ...
          {!mobile && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '5px 11px', fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
          ...
          {!mobile && (
            <button onClick={() => setAssetsOpen(true)}
              style={{ background: 'var(--card-2)', border: `1px solid var(--border)`, color: 'var(--text-muted)', padding: '6px 13px', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
          ...
          {!mobile && <WorkspaceSwitcher currentRole="creator" />}
          ...
          <div ref={avatarRef} style={{ position: 'relative', flexShrink: 0 }}>
            <div onClick={() => setAvatarOpen(o => !o)}
              ...
              <span style={{ fontSize: 11, fontWeight: 800, color: '#0d0d0d', lineHeight: 1 }}>JC</span>
            </div>

            {avatarOpen && (
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: 'var(--card)', border: `1px solid var(--border-strong)`, borderRadius: 14, overflow: 'hidden', minWidth: 220, boxShadow: 'var(--shadow-lg)', zIndex: 200 }}>
                {/* User info */}
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: LIME, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ fontSize: 11, fontWeight: 800, color: '#0d0d0d' }}>JC</span>
                    </div>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', lineHeight: 1.2 }}>Jordan Creator</p>
                      <p style={{ fontSize: 11, color: 'var(--text-faint)', lineHeight: 1.3 }}>Creator Workspace</p>
                    </div>
                  </div>
                  {/* Credits bar */}
                  <div style={{ background: 'var(--bg-soft)', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '8px 10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                      <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 500 }}>{credits} credits left</span>
                ...
                <div style={{ padding: '6px' }}>
                  {[
                    ...
                    <button key={item.label}
                      ...
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 13, fontFamily: 'inherit', textAlign: 'left', transition: 'background 0.1s, color 0.1s' }}
                      onMouseEnter={e => { const el = e.currentTarget as HTMLButtonElement; el.style.background = 'var(--card-2)'; el.style.color = 'var(--text)'; }}
      ...
      {/* ── MOBILE BOTTOM NAV ───────────────────────────────────────────────── */}
      {mobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 56, background: 'var(--nav-bg)', borderTop: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-around', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom)' }}>
          {MOBILE_NAV.map(item => {
            ...
            return (
              <Link key={item.label} href={item.href}
                ...
                <item.icon size={20} color={active ? ORANGE : 'var(--text-faint)'} strokeWidth={active ? 2.25 : 1.75} />
                <span style={{ fontSize: 10, fontWeight: active ? 700 : 500, color: active ? ORANGE : 'var(--text-faint)', letterSpacing: '0.01em' }}>
      ...
      {mobile && drawerOpen && (
        <>
          ...
          {/* Drawer */}
          <div ref={drawerRef}
            style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 240, background: 'var(--card)', borderRight: `1px solid ${BORDER}`, zIndex: 160, display: 'flex', flexDirection: 'column', padding: '16px 0', boxShadow: 'var(--shadow)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px 16px', borderBottom: `1px solid ${BORDER}` }}>
              ...
              <button onClick={() => setDrawerOpen(false)}
                style={{ width: 28, height: 28, background: 'var(--border)', border: 'none', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {NAV_LINKS.filter((l, i, arr) => arr.findIndex(x => x.href === l.href) === i).map(item => {
                ...
                return (
                  <Link key={item.label} href={item.href} onClick={() => setDrawerOpen(false)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderRadius: 10, marginBottom: 4, textDecoration: 'none', background: active ? 'rgba(255,92,0,0.1)' : 'transparent', color: active ? 'var(--text)' : 'var(--text-muted)', fontSize: 15, fontWeight: active ? 700 : 400, fontFamily: 'inherit' }}>
            ...
            <div style={{ padding: '12px 16px', borderTop: `1px solid ${BORDER}` }}>
              <button onClick={() => setAssetsOpen(true)}
                style={{ width: '100%', background: 'var(--card-2)', border: `1px solid ${BORDER}`, color: 'var(--text)', padding: '10px 14px', borderRadius: 9, fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'inherit', marginBottom: 8 }}>
                <FolderOpen size={15} />
                Assets
              </button>
              <button onClick={handleLogout}
      ...
      {upgradeOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 22, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>

            {/* Header */}
            <div style={{ padding: '24px 24px 0', borderBottom: `1px solid var(--border)`, paddingBottom: 16 }}>
              <button onClick={() => setUpgradeOpen(false)}
                style={{ position: 'absolute', top: 14, right: 14, width: 28, height: 28, borderRadius: '50%', background: 'var(--border)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <X size={14} color="var(--text-faint)" />
              </button>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 4 }}>Upgrade Plan or Buy Credits</h2>

              {/* Demo usage bar */}
              <div style={{ display: 'flex', gap: 14, marginTop: 12, flexWrap: 'wrap' }}>
                <div style={{ background: 'var(--card-2)', border: `1px solid var(--border)`, borderRadius: 10, padding: '8px 14px', flex: 1, minWidth: 120 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>Current Plan</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Demo</div>
                </div>
                <div style={{ background: 'var(--card-2)', border: `1px solid var(--border)`, borderRadius: 10, padding: '8px 14px', flex: 1, minWidth: 120 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>Credits</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: LIME }}>{credits} remaining</div>
                </div>
                <div style={{ background: 'var(--card-2)', border: `1px solid var(--border)`, borderRadius: 10, padding: '8px 14px', flex: 1, minWidth: 140 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>Per generation</div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)' }}>Video · 75cr &nbsp;|&nbsp; Image · 4cr</div>
                </div>
              </div>
                ...
                {(['plans', 'credits'] as const).map(t => (
                  <button key={t} onClick={() => setUpgradeTab(t)}
                    style={{ flex: 1, padding: '9px', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit', borderRadius: 8,
                      background: upgradeTab === t ? 'var(--border-strong)' : 'transparent',
                      color: upgradeTab === t ? 'var(--text)' : 'var(--text-faint)',
            ...
            <div style={{ padding: '20px 24px 24px' }}>
              ...
              {upgradeTab === 'plans' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[
                    { name: 'Starter',  price: 29,  credits: 300,   badge: null,           border: 'var(--border)',   btn: 'var(--card-2)', btnTxt: '#fff' },
                    ...
                    <div key={plan.name} style={{ display: 'flex', alignItems: 'center', background: 'var(--card-2)', border: `1px solid ${plan.border}`, borderRadius: 14, padding: '14px 16px', gap: 14, position: 'relative',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                          ...
                          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{plan.credits.toLocaleString()} credits/mo</span>
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-faint)' }}>
                          ~{Math.floor(plan.credits / 75)} videos · ~{Math.floor(plan.credits / 4)} images · Cancel anytime
                        </div>
                      </div>
                      <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)', flexShrink: 0 }}>${plan.price}<span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-faint)' }}>/mo</span></div>
              ...
              {upgradeTab === 'credits' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    ...
                    <div key={pack.credits} style={{ display: 'flex', alignItems: 'center', background: 'var(--card-2)', border: `1px solid ${pack.badge ? 'rgba(163,230,53,0.2)' : 'var(--border)'}`, borderRadius: 12, padding: '12px 16px', gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                          <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)' }}>{pack.credits.toLocaleString()} credits</span>
                          {pack.badge && (
                      ...
                      <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', flexShrink: 0 }}>${pack.price}</div>
                      <button onClick={() => { setComingSoon(true); setTimeout(() => setComingSoon(false), 3200); }}
              ...
              {comingSoon && (
                <div style={{ marginTop: 16, background: 'rgba(255,92,0,0.1)', border: '1px solid rgba(255,92,0,0.25)', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 2 }}>Payments coming soon</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>Stripe will be connected after UI approval. No charges will be made yet.</div>
                  </div>
                </div>
              )}
