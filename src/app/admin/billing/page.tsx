  ...
  return (
    <div style={{ padding: '28px 24px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: '-0.03em', marginBottom: 4 }}>Billing</h1>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.38)' }}>Revenue overview and subscription breakdown.</p>
      </div>
        ...
        {REVENUE_STATS.map(s => (
          <Card key={s.label} style={{ padding: '18px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontWeight: 500, lineHeight: 1.4 }}>{s.label}</p>
            ...
            <p style={{ fontSize: 24, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 5 }}>{s.value}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 14, marginBottom: 20 }}>
        ...
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: `1px solid ${BORDER}` }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>Recent Payments</span>
            <button style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>View all</button>
          </div>
          <div>
            {RECENT_PAYMENTS.map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 18px', borderBottom: i < RECENT_PAYMENTS.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>{p.plan} · {p.date}</p>
                </div>
        ...
        <Card>
          <div style={{ padding: '14px 18px', borderBottom: `1px solid ${BORDER}` }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>Plans</span>
          </div>
              ...
              <div key={i} style={{ marginBottom: i < PLANS.length - 1 ? 18 : 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                  <div>
                    ...
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginLeft: 7 }}>{p.price}</span>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', flexShrink: 0 }}>{p.users.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
