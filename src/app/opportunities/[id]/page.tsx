...
const STATUS_META: Record<string, { color: string; bg: string }> = {
  'Posted':           { color: 'rgba(255,255,255,0.5)', bg: 'rgba(255,255,255,0.07)' },
      ...
      <>
        ...
        <main style={{ paddingTop: 80, minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px' }}>
          <div style={{ textAlign: 'center', maxWidth: 420 }}>
            ...
            <h2 style={{ fontSize: 22, fontWeight: 800, color: '#fff', marginBottom: 10 }}>Project not found</h2>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', marginBottom: 24, lineHeight: 1.7 }}>That project ID doesn&apos;t exist or may have been removed.</p>
    ...
    <>
      ...
      <main style={{ paddingTop: 60, minHeight: '100vh', background: BG }}>
        ...
        <div style={{ padding: '20px 24px 0', maxWidth: 1100, margin: '0 auto' }}>
          <Link href="/opportunities" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}
        ...
        <div className="detail-layout" style={{ display: 'flex', gap: 24, padding: '24px 24px 64px', maxWidth: 1100, margin: '0 auto', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                    <h1 style={{ fontSize: 'clamp(18px, 2.5vw, 26px)', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>{project.title}</h1>
                  ...
                  <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '22px 22px', marginBottom: 16 }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 14 }}>Project Brief</h2>
              <pre style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.9, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit' }}>
                {project.brief}
              </pre>
            </div>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '22px', marginBottom: 16 }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 14 }}>Deliverables</h2>
            ...
            {/* Skills */}
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '22px', marginBottom: 16 }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 12 }}>Skills Needed</h2>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {project.skills.map(s => (
                  <span key={s} style={{ fontSize: 12, fontWeight: 600, background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.6)', padding: '5px 12px', borderRadius: 20, border: `1px solid ${BORDER}` }}>{s}</span>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '22px' }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 12 }}>Brand Assets</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {project.assets.map(a => (
                  <div key={a.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: '#1a1a1a', border: `1px solid ${BORDER}`, borderRadius: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 7, background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.35)' }}>
                      {a.type}
                    </div>
                    <div style={{ flex: 1, fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>{a.name}</div>
          ...
          <div className="sidebar" style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '22px' }}>
              <div style={{ display: 'flex', justify: 'space-between', gap: 12, marginBottom: 14 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginBottom: 4 }}>Creator Pay</div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: LIME, letterSpacing: '-0.02em' }}>${project.creatorPayMin}–${project.creatorPayMax}</div>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginBottom: 4 }}>Budget</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#fff', letterSpacing: '-0.02em' }}>${project.budget}</div>
                </div>
              </div>
              <div style={{ height: 1, background: BORDER, marginBottom: 14 }} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: '#fff', fontWeight: 600 }}>{project.contentType}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            {applied ? (
              <div style={{ background: 'rgba(163,230,53,0.07)', border: '1px solid rgba(163,230,53,0.25)', borderRadius: 14, padding: '20px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 6 }}>Application Submitted</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>Demo application saved. Track it in your Applications page.</div>
            ...
            {project.applicants.length > 0 && (
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px', fontSize: 13, color: 'rgba(255,255,255,0.45)', textAlign: 'center' }}>
                <span style={{ fontWeight: 700, color: '#fff', fontSize: 16 }}>{project.applicants.length}</span> creator{project.applicants.length !== 1 ? 's' : ''} applied
              </div>
            )}
      ...
      {modalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 22, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            <div style={{ padding: '24px 24px 0', borderBottom: `1px solid ${BORDER}`, paddingBottom: 18 }}>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 4 }}>Apply to Project</h2>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>{project.title} · {project.brandName}</p>
            </div>
            ...
            <div style={{ padding: '22px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textAlign: 'center' }}>Demo only — saved to localStorage. No real submission.</p>
            </div>
          </div>
        </div>
      )}
