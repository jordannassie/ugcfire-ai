...
export default function CreatorProjectDetailPage() {
  ...
  if (!project) {
    return (
      <div style={{ padding: '64px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>That project ID doesn&apos;t exist.</p>
        <Link href="/dashboard/projects" style={{ color: LIME, textDecoration: 'none', fontWeight: 600, fontSize: 14 }}>← Back to Projects</Link>
      </div>
    );
  }

  const statusMeta = STATUS_META[project.status] ?? { color: 'var(--text-muted)' };
  ...
  const escrowColor = project.escrowStatus === 'Released' ? LIME : project.escrowStatus === 'Not Funded' ? 'var(--text-faint)' : '#22d3ee';
  ...
  return (
    <>
      ...
      <div style={{ padding: '20px 24px 8px', maxWidth: 1060, margin: '0 auto' }}>
        <Link href="/dashboard/projects" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)', textDecoration: 'none' }}>
      ...
      <div className="proj-layout" style={{ display: 'flex', gap: 22, padding: '16px 24px 64px', maxWidth: 1060, margin: '0 auto', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 5 }}>
                  <h1 style={{ fontSize: 'clamp(17px, 2.5vw, 22px)', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em' }}>{project.title}</h1>
          ...
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px' }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>Brief</h2>
            <pre style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.9, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit' }}>
              {project.brief}
            </pre>
          </div>
          ...
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Deliverables</h2>
              <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>{itemChecked.filter(Boolean).length}/{project.deliverables.length} completed</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {project.deliverables.map((d, i) => (
                <div key={i} onClick={() => toggleCheck(i)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: itemChecked[i] ? 'rgba(163,230,53,0.05)' : 'var(--card-2)', border: `1px solid ${itemChecked[i] ? 'rgba(163,230,53,0.2)' : BORDER}`, borderRadius: 10, cursor: 'pointer', transition: 'all 0.15s' }}>
                  <div style={{ width: 20, height: 20, borderRadius: 6, border: `1.5px solid ${itemChecked[i] ? LIME : 'var(--border-strong)'}`, background: itemChecked[i] ? LIME : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
                    {itemChecked[i] && <CheckCircle size={12} color="#0d0d0d" strokeWidth={3} />}
                  </div>
                  <span style={{ fontSize: 13, color: itemChecked[i] ? 'var(--text-faint)' : 'var(--text)', textDecoration: itemChecked[i] ? 'line-through' : 'none', transition: 'color 0.15s' }}>{d}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Brand assets */}
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px' }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>Brand Assets</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {project.assets.map(a => (
                <div key={a.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 10 }}>
                  <div style={{ width: 30, height: 30, borderRadius: 7, background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: 'var(--text-faint)', flexShrink: 0 }}>{a.type}</div>
                  <span style={{ flex: 1, fontSize: 13, color: 'var(--text)', fontWeight: 500 }}>{a.name}</span>
          ...
          {submitted ? (
            <div style={{ background: 'rgba(163,230,53,0.06)', border: '1px solid rgba(163,230,53,0.25)', borderRadius: 16, padding: '24px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>Deliverable submitted (demo)</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.7 }}>In the live version, this would notify the client and update the project status to &apos;Submitted&apos;.</div>
            </div>
          ) : (
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px' }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 14 }}>Submit Deliverable</h2>
              {submitOpen ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6, display: 'block' }}>Deliverable title</label>
                  ...
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6, display: 'block' }}>Upload placeholder</label>
                    <div style={{ border: '1.5px dashed var(--border)', borderRadius: 10, padding: '24px', textAlign: 'center' }}>
                      <Upload size={20} color="var(--text-faint)" strokeWidth={1.5} style={{ margin: '0 auto 8px' }} />
                      <div style={{ fontSize: 12, color: 'var(--text-faint)' }}>Upload disabled in demo</div>
                    </div>
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6, display: 'block' }}>Notes to client</label>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <button onClick={() => setSubmitOpen(false)} style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: 10, padding: '12px 16px', color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit' }}>
                      Cancel
                    </button>
                  </div>
                </div>
        ...
        <div className="proj-sidebar" style={{ width: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Pay & escrow */}
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px' }}>
            <div style={{ fontSize: 11, color: 'var(--text-faint)', marginBottom: 4 }}>Your Pay</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}><DollarSign size={11} strokeWidth={2} />Escrow</span>
                <span style={{ color: escrowColor, fontWeight: 700 }}>{project.escrowStatus}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}><Clock size={11} strokeWidth={2} />Deadline</span>
                <span style={{ color: 'var(--text)', fontWeight: 600 }}>{project.deadline}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}><Package size={11} strokeWidth={2} />Type</span>
                <span style={{ color: 'var(--text)', fontWeight: 600, textAlign: 'right', maxWidth: 130 }}>{project.contentType}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>Client Messages</h3>
              <Link href="/dashboard/messages" style={{ fontSize: 11, color: LIME, textDecoration: 'none', fontWeight: 600 }}>View all</Link>
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 12px', lineHeight: 1.6 }}>
          ...
          {project.submissions.length > 0 && (
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '18px' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>Your Submissions</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {project.submissions.map(s => (
                  <div key={s.id} style={{ padding: '10px 12px', background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 10 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', marginBottom: 3, lineHeight: 1.3 }}>{s.title}</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 10, color: 'var(--text-faint)' }}>{s.submittedAt}</span>
