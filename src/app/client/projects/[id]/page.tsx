    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .field-input { width: 100%; background: var(--input-bg); border: 1px solid var(--border); border-radius: 9px; padding: 10px 13px; font-size: 13px; color: var(--text); outline: none; font-family: inherit; }
      `}</style>

      <div style={{ padding: '20px 24px 8px', maxWidth: 1000, margin: '0 auto' }}>
        <Link href="/client/projects" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)', textDecoration: 'none' }}>
        ...
        {activeTab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px 20px' }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 10 }}>Project Brief</h2>
              <pre style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.9, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit' }}>{project.brief}</pre>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px' }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 10 }}>Deliverables</h3>
                {project.deliverables.map((d, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6, fontSize: 12, color: 'var(--text)' }}>
                    <span style={{ color: LIME, fontWeight: 700, flexShrink: 0 }}>{i + 1}.</span>{d}
                  </div>
                ))}
              </div>
              <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px' }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 10 }}>Skills Needed</h3>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {project.skills.map(s => (
                    <span key={s} style={{ fontSize: 11, fontWeight: 600, background: 'rgba(255,255,255,0.06)', color: 'var(--text-muted)', padding: '4px 10px', borderRadius: 20, border: `1px solid ${BORDER}` }}>{s}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'applicants' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {project.applicants.length === 0 ? (
              <div style={{ textAlign: 'center', background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '40px 24px', color: 'var(--text-muted)', fontSize: 13 }}>
                ...
                <div key={a.id} style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>{a.role} · {a.portfolioCount} portfolio pieces</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6, background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 9, padding: '10px 12px' }}>
                        &ldquo;{a.message}&rdquo;
                      </div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
                        {a.skills.map(s => <span key={s} style={{ fontSize: 10, background: 'rgba(255,255,255,0.05)', color: 'var(--text-muted)', padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>{s}</span>)}
                      </div>
                    </div>
              ...
              return (
                <div key={s.id} style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '18px 20px' }}>
                  ...
                  <div style={{ height: 120, background: 'linear-gradient(135deg,#1a1a2e,#16213e)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Preview</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6, background: 'var(--card-2)', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '8px 12px' }}>
                        {s.notes}
                      </div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, flexShrink: 0 }}>
          ...
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
              ...
              {DEMO_MESSAGES.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.from === 'client' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    ...
                    background: m.from === 'client' ? ORANGE : 'var(--card-2)',
            ...
            <div style={{ padding: '14px 16px', borderTop: `1px solid ${BORDER}`, display: 'flex', gap: 10 }}>
              ...
              <button disabled style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(255,92,0,0.3)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'not-allowed', flexShrink: 0 }}>
                <Send size={16} color="var(--text-faint)" strokeWidth={2} />
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'payments' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
              {[
                ...
                <div key={c.label} style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-faint)', marginBottom: 5 }}>{c.label}</div>
            ...
            <div style={{ background: 'rgba(255,92,0,0.05)', border: '1px solid rgba(255,92,0,0.15)', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>Release Payment to Creators</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Available after you approve all submissions. Stripe wired in a future step.</div>
              </div>
